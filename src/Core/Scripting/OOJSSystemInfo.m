...
{
    return other == self || ([other isKindOfClass:[OOSystemInfo class]] && [other galaxy] == _galaxy && [other system] == _system);
}
...
- (jsval)oo_jsValueInContext:(JSContext*)context
{
    ...
    if (jsSelf != NULL) {
        if (!JS_SetPrivate(context, jsSelf, [self retain]))
            jsSelf = NULL;
    }
    if (jsSelf != NULL)
        result = OBJECT_TO_JSVAL(jsSelf);
...
static void SystemInfoFinalize(JSContext* context, JSObject* this)
{
    ...
    if (sCachedSystemInfo == this)
        sCachedSystemInfo = NULL;
...
static JSBool SystemInfoDistanceToSystem(JSContext* context, uintN argc, jsval* vp)
{
    ...
    if (!JSSystemInfoGetSystemInfo(context, OOJS_THIS, &thisInfo))
        return NO;
