...
+ (void)addExternalPath:(NSString*)path
{
    ...
    if (![sSearchPaths containsObject:path]) {
        ...
        if (sExternalPaths == nil)
            sExternalPaths = [[NSMutableArray alloc] init];
...
+ (void)checkOXPMessagesInPath:(NSString*)path
{
    ...
    if ([OXPMessageArray count] > 0) {
        ...
        if (sOXPsWithMessagesFound == nil)
            sOXPsWithMessagesFound = [[NSMutableArray alloc] init];
    ...
    if (OK) {
        ...
        if (requiredVersion != nil) {
            ...
            if ([requiredVersion isKindOfClass:[NSString class]]) {
                ...
                if (NSOrderedAscending == CompareVersions(ooVersionComponents, oxpVersionComponents))
                    OK = NO;
            } else {
    ...
    if (OK) {
        ...
        if (maxVersion != nil) {
            ...
            if ([maxVersion isKindOfClass:[NSString class]]) {
                ...
                if (NSOrderedDescending == CompareVersions(ooVersionComponents, oxpVersionComponents))
                    OK = NO;
            } else {
...
+ (void)addErrorWithKey:(NSString*)descriptionKey param1:(id)param1 param2:(id)param2
{
    if (descriptionKey != nil) {
        if (sErrors == nil)
            sErrors = [[NSMutableArray alloc] init];
                                    ...
                                    cache:(BOOL)cache
{
    ...
    if (cache && result != nil)
        [cacheMgr setObject:result forKey:cacheKey inCache:@"dictionaries"];
...
+ (NSArray*)arrayFromFilesNamed:(NSString*)fileName inFolder:(NSString*)folderName andMerge:(BOOL)mergeFiles cache:(BOOL)useCache
{
    ...
    if (!mergeFiles) {
        ...
        for (enumerator = [ResourceManager pathEnumerator]; (path = [enumerator nextObject]);) {
            ...
            // "key" property
            if ([array count] != 0 && ([[fileName lowercaseString] isEqualToString:@"nebulatextures.plist"] || [[fileName lowercaseString] isEqualToString:@"startextures.plist"]))
    ...
    if (useCache && result != nil)
        [cache setObject:result forKey:cacheKey inCache:@"arrays"];
...
+ (NSDictionary*)whitelistDictionary
{
    ...
    if (whitelistDictionary == nil) {
        ...
        if (whitelistDictionary == nil)
            whitelistDictionary = [NSNull null];

        [whitelistDictionary retain];
    }

    if (whitelistDictionary == [NSNull null])
        return nil;
...
+ (NSDictionary*)logControlDictionary
{
    ...
    if (logControl == nil)
        logControl = [NSMutableDictionary dictionary];
           ...
           usePathCache:(BOOL)useCache
{
    ...
    if (path != nil)
        result = [[[class alloc] initWithContentsOfFile:path] autorelease];

    if (result != nil && ioCache != NULL) {
        if (*ioCache == nil)
            *ioCache = [[NSMutableDictionary alloc] init];
...
+ (NSString*)stringFromFilesNamed:(NSString*)fileName inFolder:(NSString*)folderName cache:(BOOL)useCache
{
    ...
    if (path != nil)
        result = [NSString stringWithContentsOfUnicodeFile:path];

    if (result != nil && useCache) {
        if (sStringCache == nil)
            sStringCache = [[NSMutableDictionary alloc] init];
    ...
    foreach (path, paths) {
        ...
        if (![ResourceManager corePlist:@"world-scripts.plist" excludedAt:path]) {
            ...
            @try {
                ...
                if (results == nil)
                    results = [OOScript worldScriptsAtPath:path];
                if (results != nil) {
...
+ (BOOL)writeDiagnosticPList:(id)plist toFileNamed:(NSString*)name
{
    ...
    if (data == nil)
        [NSPropertyListSerialization dataFromPropertyList:plist format:NSPropertyListXMLFormat_v1_0 errorDescription:NULL];
    if (data == nil)
        return NO;
