...
- (void)checkTextureNamed:(NSString *)name inFolder:(NSString *)folder
{
	...
	else
	{
		...
		if (success)
		{
			...
			{
				OOLog(@"verifyOXP.texture.notPOT", @"----- WARNING: image %@ has non-power-of-two dimensions; it will have to be rescaled (from %zux%zu pixels to %zux%zu pixels) at runtime.", displayName, pixmap.width, pixmap.height, rWidth, rHeight);
			}
			else
			{
				OOLog(@"verifyOXP.verbose.texture.OK", @"- %@ (%zux%zu px) OK.", displayName, pixmap.width, pixmap.height);
			}
