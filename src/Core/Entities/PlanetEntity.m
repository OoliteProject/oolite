...
- (id)initFromDictionary:(NSDictionary*)dict withAtmosphere:(BOOL)atmo andSeed:(Random_Seed)p_seed
{
    ...
    if (_texture != nil)
        atmoDict = [NSDictionary dictionaryWithObjectsAndKeys:@"0", @"percent_cloud", [NSNumber numberWithFloat:[planetInfo oo_floatForKey:@"cloud_alpha" defaultValue:1.0]], @"cloud_alpha", nil];
    if (atmo)
        atmosphere = [[PlanetEntity alloc] initAsAtmosphereForPlanet:self dictionary:atmoDict];
...
- (void)setOrientation:(Quaternion)inOrientation
{
    ...
    if (atmosphere)
        [atmosphere setOrientation:inOrientation];
}
...
{
    if ([UNIVERSE breakPatternHide] || translucent || immediate)
        return; // DON'T DRAW
