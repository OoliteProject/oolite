...
- (void)setGuiToKeyMapperScreen:(unsigned)skip resetCurrentRow:(BOOL)resetCurrentRow
{
    ...
    if (resetCurrentRow) {
        int offset = 0;
        if ([[keyFunctions objectAtIndex:skip] objectForKey:KEY_KC_HEADER])
            offset = 1;
...
- (void)keyMapperInputHandler:(GuiDisplayGen*)gui view:(MyOpenGLView*)gameView
{
    ...
    if (selectKeyPress) {
        if ([key hasPrefix:@"More:"]) {
            ...
            if (from_function == 0)
                current_row = GUI_ROW_KC_FUNCSTART + MAX_ROWS_KC_FUNCTIONS - 1;
        ...
        if ([key_list count] > i) {
            ...
            if (k_int > 0) {
                ...
                if ([[def objectForKey:@"shift"] boolValue] == YES)
                    keyshift = DESC(@"oolite-keyconfig-modkey-on");
                if ([[def objectForKey:@"mod1"] boolValue] == YES)
                    keymod1 = DESC(@"oolite-keyconfig-modkey-on");
                if ([[def objectForKey:@"mod2"] boolValue] == YES)
                    keymod2 = DESC(@"oolite-keyconfig-modkey-on");
            }
        }
...
- (NSDictionary*)makeKeyGuiDict:(NSString*)what keyDef:(NSString*)key_def
{
    ...
    if ([what length] > 50)
        what = [[what substringToIndex:48] stringByAppendingString:@"..."];
...
- (void)displayKeyboardLayoutList:(GuiDisplayGen*)gui skip:(NSUInteger)skip
{
    ...
    if (n_functions > 0) {
        ...
        for (i = 0; i < (n_functions - skip) && (int)i < n_rows; i++) {
            ...
            NSString* selected = @"";
            if ([[entry objectForKey:@"key"] isEqualToString:kbd])
                selected = @"Current";
...
- (NSString*)validateKey:(NSString*)key checkKeys:(NSArray*)check_keys
{
    ...
    NSArray* docked_keys = [[NSArray arrayWithObjects:@"key_launch_ship", @"key_gui_screen_options", @"key_gui_screen_equipship", @"key_gui_screen_interfaces", @"key_gui_screen_status",
    ...
    NSArray* paused_keys = [[[NSArray arrayWithObjects:@"key_pausebutton", @"key_gui_screen_options", @"key_hud_toggle", @"key_show_fps", @"key_mouse_control_roll",
...
- (BOOL)entryIsEqualToDefault:(NSString*)key
{
    ...
    for (i = 0; i < [key_list count]; i++) {
        ...
        if (![self compareKeyEntries:orig second:entrd])
            return NO;
    }
    return YES;
}
