...
#if OO_SHADERS
    if (EXPECT_NOT(shaderMode == kShaderModeUnknown))
        [self checkShaderMode];

    // Shader takes care of repositioning.
    if (shaderMode == kShaderModeOn)
        return;
#endif
...
{
    if (!drawDust || [UNIVERSE breakPatternHide] || !translucent)
        return; // DON'T DRAW
