    ...
    for (subEnum = [self dockSubEntityEnumerator]; (sub = [subEnum nextObject]);) {
        ...
        if (sub != player_reserved_dock || [ship isPlayer]) {
            ...
            if ([docking isEqualToString:@"DOCK_CLOSED"]) {
                ...
                if (OK)
                    OK = JS_ValueToBoolean(context, rval, &tempreject);
                if (!OK)
                    tempreject = NO; // default to permreject
                if (tempreject) {
...
{
    if (self != [UNIVERSE station])
        [super adjustVelocity:xVel]; // dont get moved
}
...
{
    if (level < STATION_ALERT_LEVEL_GREEN)
...
{
    if ([shipinfoDictionary oo_boolForKey:@"rotating" defaultValue:NO])
        return YES;
