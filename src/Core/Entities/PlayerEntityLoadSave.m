...
- (void)quicksavePlayer
{
    ...
    path = save_path;
    if (!path)
        path = [[gameView gameController] playerFileToLoad];
    if (!path) {
    ...
    */
    ...
    if (loadedOK) {
        ...
        if (shipDict == nil) {
            loadedOK = NO;
            if (shipKey != nil)
                fail_reason = [NSString stringWithFormat:DESC(@"loadfailed-could-not-find-ship-type-@-please-reinstall-the-appropriate-OXP"), shipKey];
            else
    ...
    if ([[UNIVERSE gameController] finishedLaunching])
        [[UNIVERSE gameView] suppressKeysUntilKeyUp];
    if (asNew) {
        gui_screen = GUI_SCREEN_NEWGAME;
    } else {
    ...
    [self setGuiToStatusScreen];
    if (loadedOK)
        [self doWorldEventUntilMissionScreen:OOJSID("missionScreenOpportunity")]; // trigger missionScreenOpportunity immediately after loading
...
- (void)showCommanderShip:(int)cdrArrayIndex
{
    ...
    if (![cdr oo_boolForKey:@"isSavedGame"])
        return; // don't show things that aren't saved games
    ...
    if (shipDict != nil) {
        ...
        if (shipName == nil)
            shipName = [shipDict oo_stringForKey:KEY_NAME];
    } else {
