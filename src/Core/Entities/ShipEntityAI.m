...
- (void)requestDockingCoordinates
{
    ...
    if (station != nil && (distanceToStation2 < SCANNER_MAX_RANGE2 * 6.25 || dockingInstructions != nil)) {
        ...
        if (dockingInstructions != nil) {
            ...
            if (message != nil)
                [station sendExpandedMessage:message toShip:self];
        }
    } else {
...
- (void)setTakeOffFromPlanet
{
    ...
    if (the_planet) {
        _destination = HPvector_add([the_planet position], HPvector_multiply_scalar(HPvector_normal(HPvector_subtract([the_planet position], position)), -10000.0 - the_planet->collision_radius)); // 10km straight up
        desired_range = 50.0;
    } else {
...
- (void)thargonCheckMother
{
    ...
    if (mother == nil && [self group])
        mother = [[self group] leader];
...
- (void)escortCheckMother
{
    ...
    if ([mother acceptAsEscort:self]) {
        ...
        [self setOwner:self];
        if ([self group] == [mother escortGroup])
            [self setGroup:nil];
        [shipAI message:@"NOT_ESCORTING"];
    }
}
...
- (void)setPlanetPatrolCoordinates
{
    ...
    if (HPmagnitude2(r_pos) < 1000000 || patrol_counter == 0) {
        ...
        if (!the_station || ![the_station isStation])
...
- (void)recallStoredTarget
{
    ...
    if (found) {
        ...
        if (oldTarget == nil)
            DESTROY(_rememberedShip); // ship no longer exists
        [shipAI message:@"NOTHING_FOUND"];
    }
}
...
- (void)scanForNearestShipMatchingPredicate:(NSString*)predicateExpression
{
    ...
    if (function == nil) {
        ...
        if (function != nil) {
            if (scriptCache == nil)
                scriptCache = [[NSMutableDictionary alloc] init];
    ...
    if (function != nil) {
        ...
        if (![errorCache containsObject:key]) {
            ...
            if (errorCache == nil)
                errorCache = [[NSMutableSet alloc] init];
...
- (BOOL)performHyperSpaceExitReplace:(BOOL)replace toSystem:(OOSystemID)systemID
{
    ...
    if (systemID == -1) {
        ...
        for (i = 0; i < n_dests; i++) {
            if (systemID == [[sDests oo_dictionaryAtIndex:i] oo_intForKey:@"sysID"])
                break;
        }
