...
- (NSString*)processEscapePods // removes pods from cargo bay and treats categories of characters carried
{
    ...
    if ([UNIVERSE inInterstellarSpace])
        government = 1; // equivalent to Feudal. I'm assuming any station in interstellar space is military. -- Ahruman 2008-05-29
...
- (BOOL)addPassenger:(NSString*)Name start:(unsigned)start destination:(unsigned)Destination eta:(double)eta fee:(double)fee advance:(double)advance risk:(unsigned)risk
{
    ...
    if ([passengers count] >= max_passengers || [passenger_record objectForKey:Name] != nil)
        return NO;
              ...
              premium:(double)premium
{
    ...
    NSDictionary* cargo_info = [NSDictionary dictionaryWithObjectsAndKeys:
        ...
        [UNIVERSE describeCommodity:type
                             amount:qty],
    ...
    if (contractCargoUnits == UNITS_KILOGRAMS)
        cargoSpaceRequired /= 1000;
    if (contractCargoUnits == UNITS_GRAMS)
        cargoSpaceRequired /= 1000000;

    if (cargoSpaceRequired > [self availableCargoSpace])
        return NO;
...
- (void)setManifestScreenRow:(id)object inColor:(OOColor*)color forRow:(OOGUIRow)row ofRows:(OOGUIRow)max_rows andOffset:(OOGUIRow)offset inMultipage:(BOOL)multi
{
    ...
    if (disp_row < 1 || disp_row > max_rows)
        return;
    if (multi)
        disp_row++;
    GuiDisplayGen* gui = [UNIVERSE gui];
    ...
    // GUI stuff
    {
        ...
        if (shipCount > 0) {
            ...
            [gui setArray:[NSArray arrayWithObjects:DESC(@"shipyard-shiptype"), DESC(@"shipyard-price-label"),
                              DESC(@"shipyard-cargo-label"), DESC(@"shipyard-speed-label"), nil]
...
{
    if (shipKey == nil || [self dockedStation] == nil)
        return;
...
- (BOOL)buySelectedShip
{
    ...
    if (![self setCommanderDataFromDictionary:cmdr_dict])
        return NO;
