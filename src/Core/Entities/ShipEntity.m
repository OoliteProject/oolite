...
- (BOOL)setUpFromDictionary:(NSDictionary*)shipDict
{
    ...
    if (shipinfoDictionary == nil)
        shipinfoDictionary = [[NSDictionary alloc] init];
    ...
    */
    ...
    if (max_missiles > SHIPENTITY_MAX_MISSILES)
    ...
    // upgrades:
    equipment_weight = 0;
    if ([shipDict oo_fuzzyBooleanForKey:@"has_ecm"])
        [self addEquipmentItem:@"EQ_ECM" inContext:@"npc"];
    if ([shipDict oo_fuzzyBooleanForKey:@"has_scoop"])
        [self addEquipmentItem:@"EQ_FUEL_SCOOPS" inContext:@"npc"];
    if ([shipDict oo_fuzzyBooleanForKey:@"has_escape_pod"])
    ...
    if (![shipDict oo_boolForKey:@"hyperspace_motor" defaultValue:YES])
        hyperspaceMotorSpinTime = -1;
    ...
    if (octree)
        mass = (GLfloat)(density * 20.0f * [octree volume]);
    ...
    if (color == nil)
        color = [OOColor colorWithRGBAComponents:defaultExhaustEmissiveColorComponents];
    ...
    if (isWeaponNone(forward_weapon_type)) {
        ...
        if (isWeaponNone(weapon_type) && hasTurrets) { /* safety for ships only equipped with turrets
...
- (BOOL)setUpShipFromDictionary:(NSDictionary*)shipDict
{
    ...
    if (![self setUpFromDictionary:shipDict])
        return NO;
    ...
    if (accuracy < -5.0f || accuracy > 10.0f) {
        ...
        if (accuracy < 0.0f && (scanClass == CLASS_MILITARY || scanClass == CLASS_POLICE)) { // police and military pilots have a better average skill.
            accuracy = -accuracy;
        }
    }
...
- (BOOL)checkCloseCollisionWith:(Entity*)other
{
    if (other == nil)
        return NO;
    if ([collidingEntities containsObject:other])
        return NO; // we know about this already!

    ShipEntity* otherShip = nil;
    if ([other isShip])
        otherShip = (ShipEntity*)other;

    if ([self canScoop:otherShip])
        return YES; // quick test - could this improve scooping for small ships? I think so!
    ...
    if (otherShip != nil && trackCloseContacts) {
        ...
        if (![closeContactsInfo objectForKey:other_key] && HPdistance2(position, otherPos) < collision_radius * collision_radius) {
...
- (void)update:(OOTimeDelta)delta_t
{
    ...
    if (!haveExecutedSpawnAction) {
        ...
        if (script != nil && (status == STATUS_IN_FLIGHT || status == STATUS_LAUNCHING || status == STATUS_BEING_SCOOPED || (status == STATUS_ACTIVE && self == [UNIVERSE station]))) {
    ...
    // rotational velocity
    if (!quaternion_equal(subentityRotationalVelocity, kIdentityQuaternion) && !quaternion_equal(subentityRotationalVelocity, kZeroQuaternion)) {
...
- (void)respondToAttackFrom:(Entity*)from becauseOf:(Entity*)other
{
    ...
    if ([source isShip])
        [(ShipEntity*)source doScriptEvent:OOJSID("shipAttackedOther") withArgument:self];
}
...
{
    if ([self hasOneEquipmentItem:itemKey includeMissiles:includeWeapons whileLoading:loading])
        return YES;

    if (loading) {
        NSString* damaged = [itemKey stringByAppendingString:@"_DAMAGED"];
        if ([_equipment containsObject:damaged])
            return YES;
    }
    ...
    if (includeWeapons) {
        ...
        if (!isWeaponNone(weaponType)) {
            if ([self hasPrimaryWeapon:weaponType])
                return YES;
        }
    }
...
{
    if ([_equipment containsObject:itemKey])
        return YES;
    ...
    if (includeMissiles && missiles > 0) {
        unsigned i;
        if ([itemKey isEqualToString:@"thargon"])
            itemKey = @"EQ_THARGON";
        for (i = 0; i < missiles; i++) {
            if (missile_list[i] != nil && [[missile_list[i] identifier] isEqualTo:itemKey])
                return YES;
        }
    }
...
- (BOOL)hasPrimaryWeapon:(OOWeaponType)weaponType
{
    ...
    for (subEntEnum = [self shipSubEntityEnumerator]; (subEntity = [subEntEnum nextObject]);) {
        if ([subEntity hasPrimaryWeapon:weaponType])
            return YES;
    }
...
- (BOOL)hasEquipmentItem:(id)equipmentKeys includeWeapons:(BOOL)includeWeapons whileLoading:(BOOL)loading
{
    ...
    if ([equipmentKeys isKindOfClass:[NSString class]]) {
        ...
        foreach (key, equipmentKeys) {
            if ([self hasOneEquipmentItem:key includeWeapons:includeWeapons whileLoading:loading])
                return YES;
        }
    }
...
- (BOOL)canAddEquipment:(NSString*)equipmentKey inContext:(NSString*)context
{
    ...
    if (![eqType canCarryMultiple] && [self hasEquipmentItem:equipmentKey])
        return NO;
    if (![self equipmentValidToAdd:equipmentKey inContext:context])
        return NO;
...
- (BOOL)equipmentValidToAdd:(NSString*)equipmentKey whileLoading:(BOOL)loading inContext:(NSString*)context
{
    ...
    if ([eqType requiresEmptyPylon] && [self missileCount] >= [self missileCapacity] && !loading)
        return NO;
    if ([eqType requiresMountedPylon] && [self missileCount] == 0 && !loading)
        return NO;
    if ([self availableCargoSpace] < [eqType requiredCargoSpace] && !validationForDamagedEquipment && !loading)
        return NO;
    if ([eqType requiresEquipment] != nil && ![self hasAllEquipment:[eqType requiresEquipment] includeWeapons:YES whileLoading:loading])
        return NO;
    if ([eqType requiresAnyEquipment] != nil && ![self hasEquipmentItem:[eqType requiresAnyEquipment] includeWeapons:YES whileLoading:loading])
        return NO;
    if ([eqType incompatibleEquipment] != nil && [self hasEquipmentItem:[eqType incompatibleEquipment] includeWeapons:YES whileLoading:loading])
        return NO;
    if ([eqType requiresCleanLegalRecord] && [self legalStatus] != 0 && !loading)
        return NO;
    if ([eqType requiresNonCleanLegalRecord] && [self legalStatus] == 0 && !loading)
        return NO;
    if ([eqType requiresFreePassengerBerth] && [self passengerCount] >= [self passengerCapacity])
        return NO;
    if ([eqType requiresFullFuel] && [self fuel] < [self fuelCapacity] && !loading)
        return NO;
    if ([eqType requiresNonFullFuel] && [self fuel] >= [self fuelCapacity] && !loading)
        return NO;
    ...
    if ([self isPlayer]) {
        ...
        if (![eqType isAvailableToAll]) {
            ...
            if (![options containsObject:equipmentKey])
                return NO;
        }
    } else {
        if (![eqType isAvailableToNPCs])
            return NO;
    }
...
- (BOOL)addEquipmentItem:(NSString*)equipmentKey withValidation:(BOOL)validateAddition inContext:(NSString*)context
{
    ...
    if ([lcEquipmentKey isEqualToString:@"thargon"])
...
- (BOOL)removeExternalStore:(OOEquipmentType*)eqType
{
    ...
    for (i = 0; i < missiles; i++) {
        if ([[missile_list[i] identifier] isEqualTo:identifier]) {
            ...
            while (++i < missiles)
                missile_list[i - 1] = missile_list[i];
...
- (OOEquipmentType*)selectMissile
{
    ...
    if ([self isThargoid]) {
        ...
        if (![self hasAutoWeapons])
            randomSelectionChance = 0.0f;
    ...
    if (missileType == nil)
        OOLogERR(@"ship.setUp.missiles", @"could not resolve missile / mine type for ship \"%@\". Original missile role:\"%@\".", [self name], _missileRole);
...
- (float)shieldBoostFactor
{
    ...
    if ([self hasShieldBooster])
        boostFactor += 1.0f;
    if ([self hasMilitaryShieldEnhancer])
        boostFactor += 1.0f;
...
- (void)behaviour_attack_target:(double)delta_t
{
    ...
    if ([forward_weapon_real_type isTurretLaser]) {
        ...
        if (!forward_weapon_ready && !aft_weapon_ready && !port_weapon_ready && !starboard_weapon_ready) { // no usable weapons! Either not fitted or overheated

            // if unarmed
            if (isWeaponNone(forward_weapon_real_type) && isWeaponNone(aft_weapon_type) && isWeaponNone(port_weapon_type) && isWeaponNone(starboard_weapon_type)) {
...
- (void)behaviour_attack_broadside_target:(double)delta_t leftside:(BOOL)leftside
{
    ...
    if (weapon_temp > COMBAT_AI_WEAPON_TEMP_USABLE) { // will probably have more luck with the other laser or picking a different attack method
        if (leftside) {
            if (!isWeaponNone(starboard_weapon_type)) {
                behaviour = BEHAVIOUR_ATTACK_BROADSIDE_RIGHT;
            } else {
...
- (void)behaviour_attack_fly_to_target:(double)delta_t
{
    ...
    if (weapon_temp > COMBAT_AI_WEAPON_TEMP_USABLE && accuracy >= COMBAT_AI_ISNT_AWFUL && aim_tolerance * range < COMBAT_AI_CONFIDENCE_FACTOR) {
        ...
        // lasers effectively
        if (aspect < 0 || !isWeaponNone(aft_weapon_type) || !isWeaponNone(port_weapon_type) || !isWeaponNone(starboard_weapon_type)) {
            frustration = 0.0;
            behaviour = BEHAVIOUR_ATTACK_TARGET;
        }
...
- (void)behaviour_face_destination:(double)delta_t
{
    ...
    if (frustration > 15.0 / max_flight_pitch) // allow more time for slow ships.
    {
        ...
        if (flightPitch == old_pitch)
            flightPitch = 0.5 * max_flight_pitch; // hack to get out of frustration.
    }
...
- (void)behaviour_formation_form_up:(double)delta_t
{
    ...
    if (distance < last_distance) // improvement
    {
        ...
        if (frustration > 15.0) {
            if (!leadShip)
                [self noteFrustration:@"BEHAVIOUR_FORMATION_FORM_UP"]; // escorts never reach their destination when following leader.
...
- (void)behaviour_fly_to_destination:(double)delta_t
{
    ...
    if (distance < desired_range) // + collision_radius)
    {
        ...
        // do the actual piloting!!
        double confidenceFactor = [self trackDestination:delta_t retreat:NO];
        if (confidenceFactor < 0.2)
            confidenceFactor = 0.2; // don't allow small or negative values.
...
- (void)updateTrackingCurve
{
    Entity* target = [self primaryTarget];
    OOTimeAbsolute now = [UNIVERSE getTime];
    if (target == nil || reactionTime <= 0.0 || trackingCurveTimes[0] + reactionTime / 3.0 > now)
        return;
...
- (void)setScannerDisplayColorHostile1:(OOColor*)color
{
    ...
    if (color == nil)
        color = [OOColor colorWithDescription:[[self shipInfoDictionary] objectForKey:@"scanner_hostile_display_color1"]];
...
- (void)setScannerDisplayColorHostile2:(OOColor*)color
{
    ...
    if (color == nil)
        color = [OOColor colorWithDescription:[[self shipInfoDictionary] objectForKey:@"scanner_hostile_display_color2"]];
...
- (void)addSubEntity:(Entity<OOSubEntity>*)sub
{
    ...
    if (subEntities == nil)
        subEntities = [[NSMutableArray alloc] init];
    sub->isSubEntity = YES;
...
- (void)setGroup:(OOShipGroup*)group
{
    if (group != _group) {
        if (_escortGroup != _group) {
            if (self == [_group leader])
                [_group setLeader:nil];
            [_group removeShip:self];
        }
        [_group release];
        [group addShip:self];
...
{
    if (roleSet == nil)
        roleSet = [[OORoleSet alloc] initWithRoleString:primaryRole];
...
- (OOPlanetEntity*)findNearestPlanet
{
    ...
    */
    ...
    foreach (planet, [UNIVERSE planets]) {
        ...
        if ([planet planetType] == STELLAR_TYPE_MINIATURE)
            continue;
...
- (OOCargoQuantity)availableCargoSpace
{
    ...
    if (EXPECT_NOT([self cargoQuantityOnBoard] + equipment_weight >= max_cargo))
        return 0;
...
{
    double factor = 1.0;
    if (desired_speed > maxFlightSpeed && [self hasFuelInjection] && fuel > MIN_FUEL)
        factor = [self afterburnerFactor];
...
{
    if (amount < 0 || (amount == 0 && [[UNIVERSE gameController] isGamePaused]))
        return;
...
- (void)becomeExplosion
{
    ...
    @try {
        if ([self isThargoid] && [roleSet hasRole:@"thargoid-mothership"])
        ...
        if (!suppressExplosion && ([self isVisible] || HPdistance2([self position], [PLAYER position]) < SCANNER_MAX_RANGE2)) {
            ...
            BOOL add_debris = (UNIVERSE->n_entities < 0.95 * UNIVERSE_MAX_ENTITIES) && ([UNIVERSE getTimeDelta] < 0.125); // FPS > 8
...
- (void)subEntityDied:(ShipEntity*)sub
{
    if ([self subEntityTakingDamage] == sub)
        [self setSubEntityTakingDamage:nil];
...
- (BOOL)isFriendlyTo:(ShipEntity*)otherShip
{
    ...
    if ((otherShip == self) || ([self isPolice] && [otherShip isPolice]) || ([self isThargoid] && [otherShip isThargoid]) || (myGroup != nil && otherGroup != nil && (myGroup == otherGroup || [otherGroup leader] == self)) || ([self scanClass] == CLASS_MILITARY && [otherShip scanClass] == CLASS_MILITARY)) {
        isFriendly = YES;
    }
    ...
    if (retreat) // calculate jink position when flying away from target.
    {
        ...
        if (range2 < collision_radius * target->collision_radius * 100.0) // Check direction within 10 * collision radius.
        {
            ...
            if (!vector_equal(relPos, kZeroVector))
                targetDirection = vector_normal(relPos);
...
- (GLfloat)currentAimTolerance
{
    ...
    if (currentWeaponFacing == WEAPON_FACING_AFT && accuracy < COMBAT_AI_ISNT_AWFUL) { // bad shots with aft lasers
        basic_aim *= 1.3;
    } else if (currentWeaponFacing == WEAPON_FACING_PORT || currentWeaponFacing == WEAPON_FACING_STARBOARD) { // everyone a bit worse with side lasers
...
- (BOOL)fireWeapon:(OOWeaponType)weapon_type direction:(OOWeaponFacing)direction range:(double)range
{
    ...
    if (weapon_temp / NPC_MAX_WEAPON_TEMP >= WEAPON_COOLING_CUTOUT)
        return NO;
    ...
    if (![weapon_type isTurretLaser]) { // thargoid laser may just pick secondary target in this case
        if (range > randf() * weaponRange * (accuracy + 7.5))
            return NO;
        if (range > weaponRange)
            return NO;
    }
    if (![self onTarget:direction withWeapon:weapon_type])
        return NO;
...
- (BOOL)fireMainWeapon:(double)range
{
    ...
    if (isWeaponNone(forward_weapon_type)) {
        ...
        if (isWeaponNone(weapon_type) && hasTurrets) { /* no forward weapon but has turrets, so set up range calculations accordingly
                                                          note: this was hard-coded to 10000.0, although turrets have a notably
                                                          shorter range. We are using a multiplier of 1.667 in order to not change
                                                          something that already works, but probably it would be best to use
                                                          TURRET_SHOT_RANGE * COMBAT_WEAPON_RANGE_FACTOR here
...
- (BOOL)fireSubentityLaserShot:(double)range
{
    ...
    if ([parent energy] <= weapon_energy_use)
        return NO;
    if ([self shotTime] < weapon_recharge_rate)
        return NO;
    if (forward_weapon_temp > WEAPON_COOLING_CUTOUT * NPC_MAX_WEAPON_TEMP)
        return NO;
    if (range > weaponRange)
        return NO;
...
- (BOOL)fireDirectLaserShot:(double)range
{
    ...
    if (my_target == nil)
        return [self fireDirectLaserDefensiveShot];
    if (range > randf() * weaponRange * (accuracy + 5.5))
        return [self fireDirectLaserDefensiveShot];
    if (range > weaponRange)
        return [self fireDirectLaserDefensiveShot];
...
- (ShipEntity*)fireMissileWithIdentifier:(NSString*)identifier andTarget:(Entity*)target
{
    ...
    if ([UNIVERSE getTime] < missile_launch_time)
        return nil;
    ...
    while ((start.x > boundingBox.min.x - mcr) && (start.x < boundingBox.max.x + mcr) && (start.y > boundingBox.min.y - mcr) && (start.y < boundingBox.max.y + mcr) && (start.z > boundingBox.min.z - mcr) && (start.z < boundingBox.max.z + mcr)) {
    ...
    if ([missile scanClass] == CLASS_THARGOID) {
        if ([self group] == nil)
            [self setGroup:[OOShipGroup groupWithName:@"thargoid group"]];
...
- (BOOL)activateCloakingDevice
{
    if (![self hasCloakingDevice] || cloaking_device_active)
        return cloaking_device_active; // no changes.
...
- (ShipEntity*)launchEscapeCapsule
{
    ...
    */
    ...
    if (n_pods > 65)
        n_pods = 65; // maximum of 64 passengers.
    if (n_pods > 1)
        passengers = [NSMutableArray arrayWithCapacity:n_pods - 1];
    ...
    if (mainPod)
        [self doScriptEvent:OOJSID("shipLaunchedEscapePod") withArgument:mainPod andArgument:passengers];
...
- (OOCargoType)dumpItem:(ShipEntity*)cargoObj
{
    ...
    // check if start is within bounding box...
    while ((start.x > boundingBox.min.x - jcr) && (start.x < boundingBox.max.x + jcr) && (start.y > boundingBox.min.y - jcr) && (start.y < boundingBox.max.y + jcr) && (start.z > boundingBox.min.z - jcr) && (start.z < boundingBox.max.z + jcr)) {
...
- (BOOL)collideWithShip:(ShipEntity*)other
{
    ...
    if (v2b < 0.0f)
        v1a = -v1a; // in same direction as v2b
...
- (BOOL)canScoop:(ShipEntity*)other
{
    ...
    if ([cargo count] >= [self maxAvailableCargoSpace])
        return NO;
    if (scanClass == CLASS_CARGO)
        return NO; // we have no power so we can't scoop
    if ([other scanClass] != CLASS_CARGO)
        return NO;
    if ([other cargoType] == CARGO_NOT_CARGO)
        return NO;
    ...
    if (dot_product(v_forward, HPVectorToVector(loc)) < 0.0f)
        return NO; // Must be in front of us
...
{
    if ([self status] == STATUS_BEING_SCOOPED)
        return; // both cargo and ship call this. Act only once.
    desired_speed = 0.0;
...
- (void)takeEnergyDamage:(double)amount from:(Entity*)ent becauseOf:(Entity*)other weaponIdentifier:(NSString*)weaponIdentifier
{
    ...
    if (!suppressExplosion) {
        ...
        if (cascade)
            energy = 0.0; // explicit set energy to zero in case an oxp raised the energy in previous line.
    }
...
- (BOOL)abandonShip
{
    ...
    if (EXPECT([self launchEscapeCapsule] != NO_TARGET)) // -launchEscapeCapsule takes care of everything for the player
    {
        if (![self isPlayer]) {
            ...
            thrust = thrust * 0.5;
            if (thrust > 5)
                thrust = 5; // 5 is the thrust of an escape-capsule
            desired_speed = 0.0;
            if ([self group])
                [self setGroup:nil]; // remove self from group.
            if (![self isSubEntity] && [self owner])
                [self setOwner:nil]; // unset owner, but not if we are a subent
            if ([self hasEscorts]) {
                OOShipGroup* escortGroup = [self escortGroup];
                ...
                for (escortEnum = [[self escortArray] objectEnumerator]; (escort = [escortEnum nextObject]);) {
                    ...
                    if ([escort group] == escortGroup)
                        [escort setGroup:nil];
                    if ([escort owner] == self)
                        [escort setOwner:escort];
                }
...
- (BOOL)acceptAsEscort:(ShipEntity*)other_ship
{
    ...
    if ([self canAcceptEscort:other_ship]) {
        ...
        if ([escortGroup containsShip:other_ship])
            return YES;
...
- (void)refreshEscortPositions
{
    if (!_escortPositionsValid) {
        ...
        for (i = 0; i < _maxEscortCount; i++) {
            ...
            if (OK)
                OK = JSValueToVector(context, result, &_escortPositions[i]);

            if (!OK)
                _escortPositions[i] = kZeroVector;
        }
...
- (void)deployEscorts
{
    ...
    if ([self primaryTarget] == nil || _escortGroup == nil)
        return;
...
- (void)dockEscorts
{
    ...
    foreach (escort, [self escortArray]) {
        ...
        if ([escort owner] == self)
            [escort setOwner:escort];
        if (target && [target isStation])
            [escort setTargetStation:target];
...
- (void)abortDocking
{
    [[UNIVERSE findEntitiesMatchingPredicate:IsStationPredicate
                                   parameter:nil
                                     inRange:-1
                                    ofEntity:nil]
        makeObjectsPerformSelector:@selector(abortDockingForShip:)
                        withObject:self];
}
...
- (void)broadcastThargoidDestroyed
{
    [[UNIVERSE findShipsMatchingPredicate:HasRolePredicate
                                parameter:@"tharglet"
                                 ...
                                 ofEntity:self]
        makeObjectsPerformSelector:@selector(sendAIMessage:)
...
- (void)broadcastHitByLaserFrom:(ShipEntity*)aggressor_ship
{
    ...
    if ((scanClass == CLASS_NEUTRAL) || (scanClass == CLASS_STATION) || (scanClass == CLASS_BUOY) || (scanClass == CLASS_POLICE) || (scanClass == CLASS_MILITARY) || (scanClass == CLASS_PLAYER)) // only for active ships...
    {
...
- (void)broadcastMessage:(NSString*)message_text withUnpilotedOverride:(BOOL)unpilotedOverride
{
    ...
    for (i = 0; i < n_scanned_ships; i++) {
        ShipEntity* ship = scanned_ships[i];
        if (![ship isPlayer])
            [ship receiveCommsMessage:expandedMessage from:self];
    }
...
{
    if (value == (BOOL)trackCloseContacts)
        return;
...
- (void)dumpSelfState
{
    ...
    if ([self escortCount] != 0)
        OOLog(@"dumpState.shipEntity", @"Escort count: %u", [self escortCount]);
...
BOOL OOUniformBindingPermitted(NSString* propertyName, id bindingTarget)
{
    ...
    if ([bindingTarget isKindOfClass:[Entity class]]) {
        if ([entityWhitelist containsObject:propertyName])
            return YES;
        if ([bindingTarget isShip]) {
            if ([shipWhitelist containsObject:propertyName])
                return YES;
        }
        if ([bindingTarget isPlayerLikeShip]) {
            if ([playerShipWhitelist containsObject:propertyName])
                return YES;
        }
        if ([bindingTarget isVisualEffect]) {
            if ([visualEffectWhitelist containsObject:propertyName])
                return YES;
        }
    }
