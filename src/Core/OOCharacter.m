...
- (void)basicSetUp
{
    ...
    legalIndex = 0;
    if (_legalStatus > 0)
        legalIndex = (_legalStatus <= 50) ? 1 : 2;
...
- (void)setCharacterFromDictionary:(NSDictionary*)dict
{
    ...
    if ([origin isKindOfClass:[NSNumber class]] || ([origin respondsToSelector:@selector(intValue)] && ([origin intValue] != 0 || [origin isEqual:@"0"]))) {
    ...
    if ([dict oo_stringForKey:@"role"])
        [self castInRole:[dict oo_stringForKey:@"role"]];
    if ([dict oo_stringForKey:@"name"])
    ...
    if ([dict oo_stringForKey:@"script"])
        [self setCharacterScript:[dict oo_stringForKey:@"script"]];
    if ([dict oo_arrayForKey:@"script_actions"])
