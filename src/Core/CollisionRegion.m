...
- (void)addSubregionAtPosition:(HPVector)pos withRadius:(GLfloat)rad
{
    ...
    if (subregions == nil)
        subregions = [[NSMutableArray alloc] initWithCapacity:32];
...
static BOOL positionIsWithinRegion(HPVector position, CollisionRegion* region)
{
    ...
    if ((position.x < loc.x - r1) || (position.x > loc.x + r1) || (position.y < loc.y - r1) || (position.y > loc.y + r1) || (position.z < loc.z - r1) || (position.z > loc.z + r1)) {
        return NO;
    }
...
static BOOL sphereIsWithinRegion(HPVector position, GLfloat rad, CollisionRegion* region)
{
    ...
    if ((position.x - rad < loc.x - r1) || (position.x + rad > loc.x + r1) || (position.y - rad < loc.y - r1) || (position.y + rad > loc.y + r1) || (position.z - rad < loc.z - r1) || (position.z + rad > loc.z + r1)) {
        return NO;
    }
    ...
    for (i = 0; i < ent_count; i++) {
        if (uni_entities[i]->isSunlit) {
            ...
            // if we can't see it, it can't be shadowing anything important
                ...
                [uni_entities[i] isVisible] && uni_entities[i]->collision_radius >= MINIMUM_SHADOWING_ENTITY_RADIUS) {
