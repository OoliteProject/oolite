...
- (NSString*)cacheKey
{
    ...
    if (_atmoGenerator != nil)
        type = [NSString stringWithFormat:@"%@-atmo", type];
...
END:
    ...
    if (success) {
        _data = buffer;
        if (generateNormalMap)
            [_nMapGenerator completeWithData:nBuffer width:_width height:_height];
        if (generateAtmosphere)
            [_atmoGenerator completeWithData:aBuffer width:_width height:_height];
    } else {
        FREE(buffer);
        FREE(nBuffer);
        FREE(aBuffer);
