...
// There are two more modified chunks below, again marked with BEGIN OOLITE.

#include "jsversion.h"
#if JS_VERSION != 185
#error Libjs version has changed; jsutil.cpp must be updated or replaced. Headers should be checked, too.
#endif
...
/* BEGIN OOLITE */
#if 1
		OOLogWithPrefix(CFSTR("javascript.backtrace", NULL, NULL, 0, CFSTR(""), CFSTR("%s [%s +0x%X]\n"), trace->name, trace->library,
							  trace->offset);
#else
